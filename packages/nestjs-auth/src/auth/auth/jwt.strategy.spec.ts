...
describe('JWT Strategy Configuration Validation', () => {
	let mockUserLookupFn: jest.MockedFunction<(userId: string) => Promise<any>>;
	let mockAppLogger: any;
	let mockTokenValidationService: any;

	beforeEach(() => {
		mockUserLookupFn = jest.fn();
	...
	describe('JWT_SECRET validation', () => {
		it('should require JWT_SECRET', () => {
			...
			expect(() => new JWTStrategy(mockUserLookupFn, mockAppLogger, mockTokenValidationService)).toThrow('JWT configuration validation failed');
		});
		...
		it('should accept valid secrets with minimum length', () => {
			...
			process.env['JWT_EXPIRES_IN'] = '15m';
			expect(() => new JWTStrategy(mockUserLookupFn, mockAppLogger, mockTokenValidationService)).not.toThrow();
		});
