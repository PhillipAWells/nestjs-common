...
describe('CustomThrottleGuard', () => {
	...
	describe('getTracker method', () => {
		describe('IP address resolution with socket', () => {
			it('should return remoteAddress from socket property', () => {
				const mockRequest = {
					socket: {
				...
				const tracker = guard['getTracker'](mockRequest);

				expect(tracker).toBe('192.168.1.100');
			});

			it('should return remoteAddress from connection property if socket unavailable', () => {
				const mockRequest = {
					connection: {
			...
			it('should prefer socket.remoteAddress over connection.remoteAddress', () => {
				const mockRequest = {
					socket: {
						remoteAddress: '192.168.1.100',
					},
		...
		describe('IP address resolution fallback', () => {
			it('should fall back to req.ip when socket/connection unavailable', () => {
				const mockRequest = {
					ip: '203.0.113.42',
				};
			...
			it('should handle null/undefined socket gracefully', () => {
				const mockRequest = {
					socket: null,
					ip: '192.168.1.1',
				};

				const tracker = guard['getTracker'](mockRequest as any);

				expect(tracker).toBe('192.168.1.1');
			});

			it('should handle undefined remoteAddress in socket', () => {
				const mockRequest = {
					socket: {
		...
		describe('IPv6 address handling', () => {
			it('should handle IPv6 addresses from socket', () => {
				const mockRequest = {
					socket: {
						remoteAddress: '::1',
					},
	...
	describe('Edge cases', () => {
		it('should handle request with both socket and connection properties missing', () => {
		...
		it('should handle request with only partial socket object', () => {
			const mockRequest = {
				socket: {
					family: 'IPv4',
				},
		...
		it('should be consistent with the same IP address across multiple calls', () => {
			const mockRequest = {
				socket: {
			...
			const tracker1 = guard['getTracker'](mockRequest);
			const tracker2 = guard['getTracker'](mockRequest);

			expect(tracker1).toBe(tracker2);
		});

		it('should return different trackers for different IPs', () => {
			const mockRequest1 = {
				socket: {
			...
			const tracker1 = guard['getTracker'](mockRequest1);
			const tracker2 = guard['getTracker'](mockRequest2);
	...
	describe('Proxy configuration scenarios', () => {
		it('should use direct socket address when available (not spoofable)', () => {
			const mockRequest = {
				socket: {
					remoteAddress: '192.168.1.100',
				},
		...
		it('should use req.ip when socket unavailable (test environment scenario)', () => {
			const mockRequest = {
				ip: '192.168.1.100',
			};
